...
using EId = enum EFrame_ID : id_type {
    ...
    ID_setPolicy = 0x55,                    //Allows the Host to change the policies used by the NCP to make fast decisions.
    ID_getPolicy = 0x56,                    //Allows the Host to read the policies used by the NCP to make fast decisions.
    ID_invalidCommand = 0x58,

    ID_incomingRouteRecordHandler = 0x59,   //Reports the arrival of a route record command frame.
    //ID_setAddressTableRemoteEui64 =

    //ID_lookupNodeIdByEui64 = 0x60,          //
...
enum EzspDecisionId : uint8_t {
    ...
    EZSP_ALLOW_JOINS_REJOINS_HAVE_LINK_KEY = 0x04,  // Send the network key in the clear to all joining devices.
                                                    //Rejoining devices are sent the network key encrypted with their trust center link key. The trust center and any
                                                    //rejoining device are assumed to share a link key, either preconfigured or obtained under a previous policy.
    EZSP_ALLOW_PRECONFIGURED_KEY_JOINS = 0x01,  // Send the network key encrypted with the joining or rejoining device's trust center link key. The trust center
                                                //and any joining or rejoining device are assumed to share a link key, either preconfigured or obtained under a previous policy.
                                                //This is the default value for the EZSP_TRUST_CENTER_POLICY.
    EZSP_ALLOW_REJOINS_ONLY = 0x02, //Send the network key encrypted with the rejoining device's trust center link key. The trust center and any
                                    //rejoining device are assumed to share a link key, either preconfigured or obtained under a previous policy. No new devices are allowed to join.
    ...
    EZSP_CHECK_BINDING_MODIFICATIONS_ARE_VALID_ENDPOINT_CLUSTERS = 0x12, // EZSP_BINDING_MODIFICATION_POLICY decision. Allows remote nodes to set local binding entries only if the
                                                                         //entries correspond to endpoints defined on the device, and for output clusters bound to those endpoints.
