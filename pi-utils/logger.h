...
void log_init(const std::string& filename, const LLOG level=LLOG::DEBUG);
void release();
...
public:
    Logger(const std::string& filename = "/var/log/zigbee_log", const LLOG level=LLOG::DEBUG);
    virtual ~Logger();
...
private:
    ...
    std::string _logger_conf = "/var/log/zigbee.conf";
